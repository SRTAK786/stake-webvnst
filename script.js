...
const contractAddress = "0x71b66122f901c10670C818911b45B054Bcbc2e6F"; // Your contract address
...
async function approveMax() {
    ...
    try {
        ...
        const result = await contract.methods.approve(contractAddress, maxAmount)
...
async function stakeTokens() {
    ...
    try {
        ...
        const allowance = await contract.methods.allowance(accounts[0], contractAddress).call();
        ...
        // Then stake
        const result = await contract.methods.stake(amountWei, referralAddress)
            .send({ from: accounts[0] });
...
async function claimRewards() {
    ...
    try {
        const result = await contract.methods.claimRewards()
            .send({ from: accounts[0] });
...
async function updateUI() {
    ...
    try {
        ...
        if (document.getElementById('stakesList')) {
            const stakesCount = await contract.methods.getUserStakesCount(accounts[0]).call();
            const stakesList = document.getElementById('stakesList');
            stakesList.innerHTML = '';
            
            if (stakesCount > 0) {
                for (let i = 0; i < stakesCount; i++) {
                    const stake = await contract.methods.userStakes(accounts[0], i).call();
                    if (stake.isActive) {
        ...
        if (document.getElementById('yourRewards')) {
            const [vntRewards, usdtRewards] = await contract.methods.getPendingRewards(accounts[0]).call();
